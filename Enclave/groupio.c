...
int myputgrent(const struct group *p) {
  if (p == NULL) {
    return -1;
  }
  ...
  length += snprintf(result_str + length, buffer_size - length, "%s:%s:%d:", p->gr_name, p->gr_passwd, p->gr_gid);

  for (char **members = p->gr_mem; *members; members++) {
    const char *format = (*(members + 1) != NULL) ? "%s," : "%s";
    length += snprintf(result_str + length, buffer_size - length, format, *members);
  }

  length += snprintf(result_str + length, buffer_size - length, "\n");
...
static int group_put (const void *ent/* , FILE * file */)
{
	...
	return (myputgrent (gr) == -1) ? -1 : 0;
}

static int group_close_hook (void)
{
	unsigned int max_members = 100/*getdef_unum("MAX_MEMBERS_PER_GROUP", 0)*/;
...
// }
char *content = "root:x:0:\nbin:x:1:\ndaemon:x:2:\n";
// char *content;

void set_gr_content(char* s) {
	content = xstrdup(s);
}

int gr_open (int mode)
{
	return commonio_open (&group_db, mode, content);
}

/*@observer@*/ /*@null@*/void gr_locate (const char *name, void *tmp)
{
	commonio_locate (&group_db, name, tmp, 1);
}
	...
	/*@null@*/struct commonio_entry *gr2)
{
	...
	int tot = strlen(gr1->line) + strlen(gr2->line);
	new_line = malloc(tot + 2);
	/* Concatenate the 2 lines */
	if (snprintf(&new_line, "%s\n%s", gr1->line, gr2->line) == -1)
		return NULL;
