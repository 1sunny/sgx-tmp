...
static int write_all (const struct commonio_db *, int);
...
char* getNextLine(char **currentPos, char* buf) {
	if (*currentPos == NULL || **currentPos == '\0') {
		return NULL;
	}

	char *endOfLine = strchr(*currentPos, '\n');
	if (endOfLine != NULL) {
		size_t lineLength = endOfLine - *currentPos;

		strncpy(buf, *currentPos, lineLength+1);
		buf[lineLength+1] = '\0';
...
int commonio_open (struct commonio_db *db, int mode, char *content)
{
	char *buf;
	char *cp;
	char *line;
...
// 	errno = saved_errno;
	...
	while (/*db->ops->fgets (buf, buflen, db->fp) == buf*/ getNextLine(&content, buf)) {
		// while (   (strrchr (buf, '\n') == NULL)
		//        && (feof (db->fp) == 0)) {
		// 	size_t len;
		//
 ...
 */
static int write_all (const struct commonio_db *db, int flag)
	/*@requires notnull db->fp@*/
{
	...
	for (p = db->head; NULL != p; p = p->next) {
		if (p->changed) {
			...
			if (db->ops->fputs (p->line, flag) == EOF) {
				return -1;
			}
			if (db->ops->fputs ("\n", flag) == EOF) {
				return -1;
	...
	print_all(flag, db->fp);
	return 0;
}

int commonio_close (struct commonio_db *db, int flag)
{
	char buf[1024];
	int errors = 0, r;
	struct stat sb;
...
int commonio_update (struct commonio_db *db, const void *eptr)
{
	...
	if (NULL != p) {
		if (next_entry_by_name (db, p->next, db->ops->getname (eptr)) != NULL) {
			// printf (("Multiple entries named '%s' in %s. Please fix this with pwck or grpck.\n"), db->ops->getname (eptr), db->filename);
 ...
 */
/*@observer@*/ /*@null@*/void commonio_locate (struct commonio_db *db, const char *name, void* tmp, int type)
{
	struct commonio_entry *p;
  ...
  int i;
	if (type == 1) {
		struct group* tmpgr = tmp;
		struct group* grent = p->eptr;
		tmpgr->gr_gid = grent->gr_gid;
		// tmpgr->gr_name = strdup (grent->gr_name);
		// tmpgr->gr_passwd = strdup (grent->gr_passwd);
		strncpy(tmpgr->gr_name, grent->gr_name, strlen(grent->gr_name) + 1);
		strncpy(tmpgr->gr_passwd, grent->gr_passwd, strlen(grent->gr_passwd) + 1);
		for (i = 0; grent->gr_mem[i]; i++);
		// tmpgr->gr_mem = MALLOC(i + 1, char *);
		for (i = 0; grent->gr_mem[i]; i++) {
			// tmpgr->gr_mem[i] = strdup (grent->gr_mem[i]);
			strncpy(tmpgr->gr_mem[i], grent->gr_mem[i], strlen(grent->gr_mem[i]) + 1);
		}
		tmpgr->gr_mem[i] = NULL;
	} else if (type == 2) {
		struct sgrp* sg = tmp;
		struct sgrp* sgent = p->eptr;
		// sg->sg_name = strdup (sgent->sg_name);
		// sg->sg_passwd = strdup (sgent->sg_passwd);
		strncpy(sg->sg_name, sgent->sg_name, strlen(sgent->sg_name) + 1);
		strncpy(sg->sg_passwd, sgent->sg_passwd, strlen(sgent->sg_passwd) + 1);
		for (i = 0; NULL != sgent->sg_adm[i]; i++);
		// sg->sg_adm = MALLOC(i + 1, char *);
		for (i = 0; NULL != sgent->sg_adm[i]; i++) {
			// sg->sg_adm[i] = strdup (sgent->sg_adm[i]);
			strncpy(sg->sg_adm[i], sgent->sg_adm[i], strlen(sgent->sg_adm[i]) + 1);
		}
		sg->sg_adm[i] = NULL;
		...
		for (i = 0; NULL != sgent->sg_mem[i]; i++) {
			...
			strncpy(sg->sg_mem[i], sgent->sg_mem[i], strlen(sgent->sg_mem[i]) + 1);
		}
		sg->sg_mem[i] = NULL;
	}
	// return p->eptr;
